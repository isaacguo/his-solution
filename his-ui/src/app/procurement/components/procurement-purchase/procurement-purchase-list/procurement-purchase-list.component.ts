...
export class ProcurementPurchaseListComponent implements OnInit {
  ...
  onCreateNewRequestButtonClicked() {
    this.router.navigate(['procurement','procurement-purchase', OperationEnum.CREATE]);
  }

  onUpdateButtonClicked(procurement: Procurement) {
    this.router.navigate(['procurement','procurement-purchase', OperationEnum.UPDATE, procurement.id]);
  }
