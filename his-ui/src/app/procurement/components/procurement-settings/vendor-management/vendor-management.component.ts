...
export class VendorManagementComponent implements OnInit, OnChanges {
  ...
  onCreateNewVendorClicked() {
    this.router.navigate(['procurement', 'procurement-settings', 'vendors', OperationEnum.CREATE, this.categoryId]);
  }

  onModifyVendorButtonClicked(vendor: Vendor) {

    this.router.navigate(['procurement', 'procurement-settings', 'vendors', OperationEnum.UPDATE, vendor.id]);
